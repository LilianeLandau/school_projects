...
if (isset($_POST['send'])) {
    ...
    if (empty($errors)) {
        ...
        // Vérifier utilisateur existe déjà
        //Requête select - on cherche si le nom existe
        $sqlQuery = 'SELECT * FROM utilisateur WHERE nom = :nom';
        //préparer la requête
        $statement = $mysqlClient->prepare($sqlQuery);
        //lier le paramètre :nom à la variable $nom issue du $_POST['nom']
        $statement->bindParam(':nom', $nom);
        //exécuter la requête
        $statement->execute();
        //si l'utilisateur existe, alors il est sauvegardé
        //dans la variable $utilisateurExiste
        $utilisateurExiste = $statement->fetch(PDO::FETCH_OBJ);

        //si l'utilisateur existe déjà dans la base
        if ($utilisateurExiste) {
            // Afficher un message d'erreur
            $errors[] = "<p class='text-danger'>Vous êtes déjà inscrit " . htmlspecialchars($nom) . " ! Vous pouvez directement vous connecter</p>";
        } else {
            //sinon, on peut continuer la procédure d'inscription
            //par sécurité, d'abord appliquer le hash au mot de passe
            ...
            //préparer la requête
            $Statement = $mysqlClient->prepare($sqlQuery);
            //lier les paramètres :nom et :mot_de_passe aux variables
            $Statement->bindParam(':nom', $nom);
            ...
            <p>Connectez-vous pour profiter de toutes les fonctionnalités du site !</p>";
                ...
                $errors[] = "<p class='text-danger'>Nous sommes désolés. Une erreur s'est produite. Merci d'essayer à nouveau.</p>";
            }
        }
    }
}
